...
local function part_one(txt)
  ...
  local function get_pos(vec)
    local char = data[vec[1]][vec[2]]
  ...
  local function get_connection(pos, dir)
    local offset = get_offset(pos, dir)
    local char = get_pos(offset)
    if not char then
      return nil
    end
    if is_opposite(dir, chars[char][1]) then
      return {
        char = char,
        pos = offset,
        i = chars[char][1],
    ...
    elseif is_opposite(dir, chars[char][2]) then
      return {
        char = char,
        pos = offset,
        i = chars[char][2],
  ...
  for _, dir in pairs(dirs) do
    local node = get_connection(start, dir)
    if node then
  ...
  if #nodes ~= 2 then
    vim.print(nodes)
    error("Error fiding starting nodes")
  end
  local result = 0
  local a = nodes[1]
  local b = nodes[2]
  while true do
    ...
    -- vim.print(a)
    -- vim.print(b)
    if is_equal(a.pos, b.pos) then
      break
    end
    local next_a = get_connection(a.pos, a.o)
    local next_b = get_connection(b.pos, b.o)
    if is_equal(a.pos, next_b.pos) then
      break
    end
    a = next_a
    b = next_b
