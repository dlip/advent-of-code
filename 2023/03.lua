...
local function part_one(schematic)
  local lines = {}
  for line in schematic:gmatch("[^\r\n]+") do
    lines[#lines + 1] = "." .. line .. "."
  end
  -- Pad lines to make it easy to cut out surroundings
  local width = lines[1]:len()
  table.insert(lines, 1, ("."):rep(width))
  table.insert(lines, ("."):rep(width))

  -- vim.print(lines, width, height)
  local numbers = {}
  local total = 0
  for line_no, line in pairs(lines) do
    ...
    for i = 1, #line do
      ...
      elseif num_start then
        local surrounding = lines[line_no - 1]:sub(num_start - 1, i)
            .. line:sub(num_start - 1, i)
            .. lines[line_no + 1]:sub(num_start - 1, i)
        local has_symbols = surrounding:gmatch("[^%w%.]")()
        -- print(surrounding)
        -- print(has_symbols)
        if has_symbols then
